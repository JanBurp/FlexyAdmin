 ...
 *
 * Changes by Jan den Besten (3/07/2010)
 * - Changed hours from 00:00 to 00
 * - added datepicker class (top div & header)
 * - Removed empy <tr>
 * - Changed output (added seconds)
...
(function($) {
	...
	_setTime = function(end){
		    ...
		    oTime.html(result);
		    if(end){
					result=result.replace(/ /g,''); // JdB
					result=result+':00'; // JdB
		      $(oInput).val(result);
			  	complete=true;
					$(oInput).trigger('change'); // JdB
		    }
				...
				if(obj.hourFormat==24){
					...
					for(var t=0;t<lengte;t++){
						...
						if( typeof(obj.hour24[t]) ==='string' ){
							...
							hourHTML+='<td><a rel="h_24_'+obj.hour24[t]+'">'+obj.hour24[t]+'</a></td>';
						}
					...
					for(var t=0;t<lengte;t++){
						if( typeof(obj.hourAM[t]) ==='string' ){
							...
							hourHTML+='<td><a rel="h_am_'+obj.hourAM[t]+'">'+obj.hourAM[t]+'<i>am</i></a></td>';
						}
					...
					for(var t=0;t<lengte;t++){
						if( typeof(obj.hourPM[t]) ==='string' ){
							...
							hourHTML+='<td><a rel="h_pm_'+obj.hourPM[t]+'">'+obj.hourPM[t]+'<i>pm</i></a></td>';
						}
						if(cr==obj.hourCols){
							hourHTML+='</tr><tr>';
							cr=0;
	...
	_drawTable = function(){
		   var obj=oOptions[oInput.identifier],
	           ...
	           header =     '\n<div class="ui-timepicker-header ui-widget-header ui-helper-clearfix ui-corner-all">'
                            +'<a class="ui-timepicker-close ui-datepicker-close ui-corner-all" title="Close" id="btn_close"><span class="ui-icon ui-icon-circle-close">Close</span></a>'
                            +'<div class="ui-timepicker-title ui-datepicker-title">'
