...
/**
 * GoogleMapAPI V3: a PHP library inteface to the Google Map API v3
 * 
 ...
 *
 * CREATE TABLE log_geocode_cache (
...
class Gmap {
     ...
     * @var string
     */
    var $_db_cache_table = 'log_geocode_cache';		// changed by JdB: to nicer name in FlexyAdmin
	...
	*/
    ...
    function putCache($address, $lon, $lat) {
		...
		$data = array(
					'query' => $_address,
					'lat' => str_replace(',','.',$lat),			// Changed by Jdb to replace the , with a . so no db problems occur
					'lng' => str_replace(',','.',$lon)
					);
