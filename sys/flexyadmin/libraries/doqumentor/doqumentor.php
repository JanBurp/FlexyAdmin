...
class Doqumentor {
  ...
  public function doc() {
    ...
    // Helper functions
    $doc['functions']=$this->docFunctions($this->functions);
    $doc['functions']=sort_by($doc['functions'],'file');
  ...
  private function docComment($obj) {
    ...
    if (!empty($doc)) {
      ...
      $p->parse();
      $params = $p->getParams();
      $params['description']= $p->getDesc();
      $params['shortdescription']= $p->getShortDesc();
      return $params;
    }
    return $doc;
  }
