				...
				case 'changedFields'	:
					...
					if (!empty($fields)) {
						...
						foreach ($fields as $field) {
							if (isset($this->newData[$field]) and $this->newData[$field]!=$this->oldData[$field]) $changedFields=TRUE;
						}
						if ($changedFields) {
							$this->actOn[$key]['act']=true;
							$this->act=true;
				...
				case 'changedTypes'		:
					...
					if (!empty($types)) {
						...
						foreach ($this->fields as $field) {
							...
							if (in_array($pre,$types) and ($this->newData[$field]!=$this->oldData[$field]) ) $changedFields=true;
						}
						if ($changedFields) {
							$this->actOn[$key]['act']=true;
							$this->act=true;
