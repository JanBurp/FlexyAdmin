...
class MY_DB_mysqli_driver extends CI_DB_mysqli_driver {
  ...
  public function table_status($table) {
	 ...
	 * 
	 * DEPRICATED -> MOVED TO Data_Core.php
	 * 
...
//					$F->collation   = $field->Collation;
  ...
  public function get_first_field($table,$pre="str") {
    ...
    * @author Jan den Besten
	 ...
	 * ORDER BY
	 * 
	 * Zelfde als CodeIgniter origineel, maar bewaart de last->order
	 *
   ...
   * Test of tweede parameter een array is, als dat zo is roep dan where_in aan
    ...
    if (isset($value) and is_array($value)) {
      return parent::where_in($key,$value,$escape);
    }
    ...
    */
	public function get_query($table="",$limit=0,$offset=0) {
		...
		/**
			...
			*/
		foreach ($this->qb_dont_select as $dont) {
			foreach ($select as $key => $value) {
				if ($value==$dont or $value=="$table.$dont") unset($select[$key]);
			}
		}
    if (!in_array(PRIMARY_KEY,$this->qb_select) and !in_array(PRIMARY_KEY,$select) and !in_array($table.'.'.PRIMARY_KEY,$this->qb_select) and !in_array($table.'.'.PRIMARY_KEY,$select)) {
    ...
    /**
      ...
      $foundKeysArray=array_ereg_search("txt_", $this->qb_select);
      foreach($foundKeysArray as $key) {
        $field=$this->qb_select[$key];
        if (!has_string('CONCAT',$field))
          $this->qb_select[$key]="SUBSTRING(".$field.",1,".$this->maxTextLen.") AS `".remove_prefix($field,".")."`";
        else {
          $this->qb_select[$key]=preg_replace("/(CONCAT(.*)\\))/uiUsm", "SUBSTRING($1,1,".$this->maxTextLen.")", $this->qb_select[$key]);
        }
      }
		}
   ...
   */
     ...
     */
		$this->qb_last_count=count($result);
