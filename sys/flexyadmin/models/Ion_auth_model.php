...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			if ($this->bcrypt->verify($password,$hash_password_db->gpw_password))
			{
				return TRUE;
			}
		...
		{
			$salt = substr($hash_password_db->gpw_password, 0, $this->salt_length);

			$db_password =  $salt . substr(sha1($salt . $password), 0, -$this->salt_length);
		}

		if($db_password == $hash_password_db->gpw_password)
		{
			return TRUE;
		}
		else
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('id, gpw_password, salt')
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$data = array(
			    'gpw_password' => $hashed_new_password,
			    'remember_code' => NULL,
			);
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$data = array(
		    ...
		    'created_on' => time(),
		    'b_active'     => ($manual_activation === false ? 1 : 0)
		);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email_email, id, gpw_password, b_active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->identity_column, $data) || array_key_exists('gpw_password', $data) || array_key_exists('email_email', $data))
		{
			if (array_key_exists('gpw_password', $data))
			{
				if( ! empty($data['gpw_password']))
				{
	...
	**/
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, email_email, last_login')
