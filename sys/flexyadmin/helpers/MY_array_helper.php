...
function array2csv($array, $eol = "\r\n")
{
    ...
    foreach (array_keys($firstLine) as $key) {
        $csv .= $key . $comma;
    }
...
function array2xml($array, $keys = NULL, $attr = NULL, $tabs = 0)
{
    ...
    foreach ($array as $key => $value) {
        ...
        if (isset($keys[$tabs])) {
            $at = '';
            if (isset($attr[$keys[$tabs]])) {
                foreach ($attr[$keys[$tabs]] as $a => $v) {
                    $at .= ' ' . $a . '="' . $v . '" ';
                }
            }
            $sub .= "<$keys[$tabs]$at>";
...
function ignorecase_ksort(&$a)
{
    $n = array();
    foreach ($a as $k => $v) {
        $n[strtolower($k)] = array("key" => $k, "value" => $v);
    }
    ksort($n, SORT_STRING);
    $a = array();
    foreach ($n as $k => $v) {
...
function array_set_multi_key(&$a, $multikey, $value)
{
    switch (count($multikey)) {
        case '1':
            ...
            break;
        case '2':
            $a[$multikey[0]][$multikey[1]] = $value;
            break;
        case '3':
            $a[$multikey[0]][$multikey[1]][$multikey[2]] = $value;
            break;
        case '4':
            $a[$multikey[0]][$multikey[1]][$multikey[2]][$multikey[3]] = $value;
            break;
        case '5':
            $a[$multikey[0]][$multikey[1]][$multikey[2]][$multikey[3]][$multikey[4]] = $value;
            break;
        case '6':
            $a[$multikey[0]][$multikey[1]][$multikey[2]][$multikey[3]][$multikey[4]][$multikey[5]] = $value;
            break;
    }
    return $a;
}
