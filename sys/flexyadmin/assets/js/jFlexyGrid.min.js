...
function doGrid(){$(".upload").click(function(){var path=get_subclass("path_",$(this));dialog.html('<div id="uploader"></div>');$(dialog).dialog({title:langp("dialog_title_upload"),modal:true,width:600,close:function(){$(dialog).dialog("destroy");}});var uploader=$("#uploader").plupload({runtimes:'html5,flash,silverlight',url:site_url("admin/filemanager/upload/"+path),max_file_size:'100mb',chunk_size:'1mb',unique_names:true,filters:[{title:"--",extensions:config.file_types},],flash_swf_url:'sys/jquery/plugins/plupload/js/plupload.flash.swf',silverlight_xap_url:'sys/jquery/plugins/plupload/js/plupload.silverlight.xap',init:{UploadComplete:function(obj,files){window.location.reload();}}});});isSortable=isGrid;if(isGrid||isFile){if($("table.grid").width()<600){$("table.grid").css({width:"600px"});}
