...
if(isGrid&&!isFile){var gridEl=$('.grid table');var edit_types=$(gridEl).attr('data-edit_types');if(edit_types){var edit_types_classes=edit_types.split(',');$.each(edit_types_classes,function(i,val){var editable_cells=$('td.'+val,gridEl);$(editable_cells).addClass('editable_cell')
switch(val){case'b':case'is':case'has':$(editable_cells).click(function(){var obj=this;var cell=get_cell($(this));var id=cell.id;var value=0;if($(this).children("div:first").hasClass("no"))value="1";var url=site_url("admin/ajax/edit/"+cell.table+"/"+cell.id+"/"+cell.field+"/"+value);$(this).css('cursor','wait');$.post(url,"",function(data){if(data!=""){ajaxError(data);}
else{var html=$(obj).html();if(value=="1")
html=html.replace(/no/g,"yes");else
html=html.replace(/yes/g,"no");$(obj).html(html);}
$(obj).css('cursor','pointer');});})
break;default:$(editable_cells).attr('contenteditable','true');$(editable_cells).focusin(function(){var obj=this;var old_value=$(this).text();$(this).focusout(function(){var new_value=$(this).text();if(new_value!=old_value){var cell=get_cell($(this));var url=site_url("admin/ajax/edit/"+cell.table+"/"+cell.id+"/"+cell.field+"/"+new_value);$(obj).css('cursor','wait');$.post(url,"",function(data){if(data!=""){ajaxError(data);$(obj).text(old_value);}
$(obj).css('cursor','pointer');});}});});break;}});}
...
function doAction(obj){$(obj).children('td.id:first').html('<div class="icon wait" /></div>');uri=$(obj).children('td.uri').html();if(uri!=''){$.ajax({type:"POST",url:uri,async:'false',success:function(data){if(data!=""&&data.indexOf('TRACE')>=0){ajaxError(data);}
else{if(data.indexOf('_message')>=0){data=$.parseJSON(data);$(obj).children('td._message').html(data._message);}
