...
$("table.grid tbody tr").removeClass("current");},sort:function(event,ui){if(startPos==0){startPos=ui.position.left;}},stop:function(event,ui){show_branches(id);$(ui.item).children().removeClass("foldednode");},update:function(event,ui){table=$("table.grid").attr("class");table=table.replace("grid","");table=table.replace("pagination","");table=table.replace("editable","");table=$.trim(table);id=get_id($(ui.item));endPos=ui.position.left;shifted=(endPos-startPos)/25;if($("table.grid tr td.self_parent").length>0){nextRow=$(ui.item).next("tr");if(nextRow.length>0){newParentId=get_subclass("parent_id_",$(nextRow));if(newParentId=="")newParentId=0;}
else
...
var url=site_url("admin/ajax/edit/");var data={'table':table,'id':id,'field':'self_parent','value':newParentId};$.post(url,data,function(data){if(data._error)ajaxError(data._error);},'json');}}
show_branches(id);$(ui.item).addClass("current");var url=site_url("admin/ajax/order/");var ser=serialize("table.grid tbody tr");var data={'table':table,'ids':ser};$.post(url,data,function(data){if(data._error)
ajaxError(data._error);else
...
function doAction(obj){$(obj).children('td.id:first').html('<div class="icon wait" /></div>');uri=$(obj).children('td.uri').html();if(uri!=''){$.ajax({type:"POST",url:uri,async:'false',dataType:'json',success:function(data){if(data._error){ajaxError(data._error);}
