...
else{$(".upload").click(function(){var path=get_subclass("path_",$(this));dialog.html('<div id="uploader"></div>');$(dialog).dialog({title:langp("dialog_title_upload"),modal:true,width:600,open:function(){var uploader=$("#uploader").plupload({runtimes:'html5,html4',url:site_url("admin/filemanager/upload/"+path),max_file_size:'10mb',filters:[{title:"--",extensions:config.file_types}],init:{UploadComplete:function(obj,files){window.location.reload();}}});$('#uploader_browse').click(function(){$('#uploader > div.plupload').css({'z-index':'10000'});$('#uploader > div.plupload').trigger('click');});},close:function(){$(dialog).dialog("destroy");}});});}
