...
fileObj=$(this).parents('tr:first');var filename=$(fileObj).children('.name:first').text();path=$(fileObj).children('.thumb:first').children('.path:first').text();ext=get_ext(filename);shortName=filename.replace('.'+ext,'');var filedate=$(fileObj).find('.date:first .hidden').text();filedate=filedate.replace(/ /g,'-');var dialogHtml='<form method="post" action="'+site_url()+'admin/filemanager/rename/'+pathencode(path)+'/current/'+filename+get_current_grid_page_uri()+'">'+'<input id="name" name="name" value="'+shortName+'" />.'+ext+'<br/>'+'<input type="hidden" name="ext" value="'+ext+'"/>'+'<input type="hidden" name="path" value="'+path+'"/>';if(filedate!='')dialogHtml+='<br/><input id="date" name="date" value="'+filedate+'"/>';dialogHtml+='</form>';dialog.html(dialogHtml);$('input#date').datepicker({dateFormat:'yy-mm-dd'});$('#ui-datepicker-div').css({'z-index':2000});$(dialog).dialog({title:langp('dialog_title_rename',filename),modal:true,width:500,buttons:({cancel:function(){$(dialog).dialog("destroy");},ok:function(){$('.ui-dialog .ui-dialog-buttonpane').add('.ui-dialog a').hide();$('.ui-dialog .ui-dialog-content').append("<img src='"+site_url("sys/flexyadmin/assets/icons/wait.gif")+"' align='right' />");newName=$('.ui-dialog input#name').attr('value')+'.'+ext;var newdate=$('.ui-dialog input#date').attr('value');if(newdate!=undefined&&newdate!=''){filedate=newdate;}
location.replace(site_url('admin/filemanager/edit/'+pathencode(path)+'/current/'+filename+'/new/'+newName+'/date/'+filedate+get_current_grid_page_uri()));}}),close:function(){$(dialog).dialog("destroy");}});changeButt("cancel",lang("dialog_cancel"));changeButt("ok",lang("dialog_ok"));});}}
