...
class TableModelTest extends CITestCase {
  ...
  public function test_db() {
    ...
    $result = $this->CI->data_model->list_tables();
    $this->assertInternalType( 'array', $result);
    
    // tbl_menu
    $this->CI->data_model->table( 'tbl_menu' );
    // ->list_fields()
    $this->assertEquals( array('id','order','self_parent','uri','str_title','txt_text','medias_fotos','b_visible','str_module','stx_description','str_keywords'), $this->CI->data_model->list_fields() );
    // ->get()
    ...
    // tbl_links
    $this->CI->data_model->table('tbl_links');
    // ->list_fields()
    $this->assertEquals( array('id','str_title','url_url'), $this->CI->data_model->list_fields() );
    // ->get()
    $query = $this->CI->data_model->get();
    ...
    $this->assertEquals( array('id','order','self_parent','uri','str_title','txt_text','medias_fotos','b_visible','str_module','stx_description','str_keywords'), $this->CI->data_model->list_fields() );
    // ->get(2)
    $query = $this->CI->data_model->get(2);
    $this->assertEquals( 2, $query->num_rows() );
    $this->assertEquals( 11, $query->num_fields() );
    // ->where()
    $query = $this->CI->data_model->where( 'order <=', '2' )->get();
    $this->assertEquals( 3, $query->num_rows() );
  }
  ...
  public function test_settings() {
    $this->CI->data_model->table( 'tbl_menu' );
    // null
    $this->assertEquals( null, $this->CI->data_model->get_setting( 'not_a_setting' ) );
    // one settings
    $this->assertEquals( 'tbl_menu', $this->CI->data_model->get_setting( 'table' ) );
    $this->assertEquals( array('id','order','self_parent','uri','str_title','txt_text','medias_fotos','b_visible','str_module','stx_description','str_keywords'), $this->CI->data_model->get_setting( 'fields' ) );
    $this->assertEquals( array( 'str_title','str_module'), $this->CI->data_model->get_setting( 'abstract_fields' ) );
    $this->assertEquals( 'order', $this->CI->data_model->get_setting( 'order_by' ) );
    // array of settings
    $this->assertEquals( array('table'=>'tbl_menu','order_by'=>'order'), $this->CI->data_model->get_setting( array('table','order_by') ) );
  }
  ...
  public function test_abstractfields() {
    ...
    $this->CI->data_model->table( 'tbl_menu' );
    // ->get_abstract_fields()
    $abstract_fields = $this->CI->data_model->get_abstract_fields();
  ...
  public function test_get_options() {
    // tbl_menu.str_module
    $this->CI->data_model->table( 'tbl_menu' );
    $options = $this->CI->data_model->get_options( 'str_module' );
    ...
    // tbl_menu
    $options = $this->CI->data_model->get_options();
    $this->assertInternalType( 'array', $options );
  ...
  public function test_setting_with() {
    ...
    // ->get_with()
    $with = $this->CI->data_model->get_with();
    ...
    // ->get_with()
    $this->assertEquals( array(), $this->CI->data_model->get_with() );
    // ->with( 'many_to_one', array( 'tbl_adressen') );
    $expected = array(
      'many_to_one'=>array(
        'tbl_adressen' => array('fields'=>array('id','str_address','str_zipcode','str_city'),'grouped'=>false,'flat'=>false),
  ...
  public function test_many_to_one_data() {
    $this->CI->data_model->table( 'tbl_leerlingen' );
    $grid_set = $this->CI->data_model->get_setting('grid_set');
