...
{
    protected $tmp_dir;
    protected $application_dir='../flexyadmin';
        ...
        $this->packages = array(
            ...
            'ion-auth' => array(
                ...
                'dir'   => array(
                    ...
                    'libraries',
                    'models',
                    'userguide' => '../../userguide/CodeIgniter-IonAuth2',
                    // 'views',
                    // 'sql' => '../../db',
                    //'migrations',
                ),
                'replace' => array(
                    'ion_auth.sql' => array(
                      ...
                      "`company`"         => "`str_company`",
                      "`phone`"           => "`str_phone`",
                      "'admin@admin.com'" => "'info@flexyadmin.com'",
                      "`users_groups`"    => "`rel_users_groups`",
                      "'users_groups'"    => "'rel_users_groups'",
                      "`user_id`"         => "`id_user`",
                      "`group_id`"        => "`id_user_group`",
                      "`login_attempts`"  => "`log_login_attempts`",
                      "'login_attempts'"  => "'log_login_attempts'",
                    ),
                    
                  'config/ion_auth.php' => array(
                    "= 'users'"           => "= 'cfg_users'",
                    "= 'groups'"          => "= 'cfg_user_groups'",
                    "= 'users_groups'"    => "= 'rel_users__groups'",
                    "= 'login_attempts'"  => "= 'log_login_attempts'",
                    "= 'user_id'"         => "= 'id_user'",
                    "= 'group_id'"        => "= 'id_user_group'",
                    "= 'email'"           => "= 'str_username'",
                  ...
                  'libraries/Ion_auth.php' => array(
                    "'active'"  => "'b_active'",
                  ),
                  
                  'models/Ion_auth_model.php' => array(
                    "'email'"                    => "'email_email'",
                    " email,"                    => " email_email,",
                    ...
                    "'password,"                 => "'gpw_password,",
                    " password, "                => " gpw_password, ",
                    "hash_password_db->password" => "hash_password_db->gpw_password",
                    "user->password, "           => "user->gpw_password, ",
                    "'active'"                   => "'b_active'",
                    " active, "                  => " b_active, ",
                    "user->active"               => "user->b_active",
                  ),
                ),
    ...
    public function install($package, $version)
    {
        ...
        $replace=FALSE;
        if (isset($this->packages[$package]['replace'])) $replace = $this->packages[$package]['replace'];

        $this->recursiveCopy($src, $dst, $replace);
    ...
    private function downloadFromGithub($package, $version)
    {
        ...
        if (is_string($dir)) {
            ...
            $dst = realpath(__DIR__ . '/'.$this->application_dir."/$dir");
            var_dump(__DIR__ . '/'.$this->application_dir."/$dir");
            return array($src, $dst);
        }
        
        foreach ($dir as $original=>$directory) {
          if (is_numeric($original)) $original=$directory;
          $src[] = realpath(dirname($filepath) . "/$repos-$version/$pre$original");
          @mkdir(__DIR__ . '/'.$this->application_dir."/$directory");
          $dst[] = realpath(__DIR__ . '/'.$this->application_dir."/$directory");
        }
        return array($src, $dst);
    }
     ...
     * @param string $dst
     */
    private function recursiveCopy($src, $dst, $replace=FALSE )
    {
        if ($src === false) {
            return;
        }
        ...
        if (is_array($src)) {
            foreach ($src as $key => $source) {
                $this->recursiveCopy($source, $dst[$key], $replace);
            }
        ...
        foreach ($iterator as $file) {
            if ($file->isDir()) {
                ...
                if ($success) {
                    ...
                    // REPLACE
                    if (is_array($replace)) {
                      foreach ($replace as $replace_file => $search_replace) {
                        $source = $dst.'/'.$iterator->getSubPathName();
                        if ( strpos($source,$replace_file)>0 ) {
                          $content = file_get_contents($source);
                          foreach ($search_replace as $s => $r) {
                            $content = str_replace($s,$r,$content);
                          }
                          file_put_contents($source,$content);
                          echo '-> replaced items in ' . $dst . '/' . $iterator->getSubPathName() . PHP_EOL;
                        }
                      }
                    }
                    //
