...
class Comments extends CI_Model {
	...
	function main($item) {
		...
		if ($form->validation()) {
			...
			// Check for spam
			$spam=FALSE;
			if (!$spam and $this->_check_if_robot($data)) 				$spam=TRUE;	// Check if a robot has filled 'body' (which is hidden)
			if (!$spam and $this->_check_if_double($data,$table))	$spam=TRUE; // Check if the message is double
			if (!$spam and $this->_check_if_spamtext($data))			$spam=TRUE; // Check the text with FlexyAdmins spam checker
			if ($spam) {
				$errorHtml.='<p class="error">Je reactie is gekenmerkt als spam en wordt niet geplaatst.</p>';
			}
			else {
				// Plaats bericht
				foreach ($data as $field => $value) {$this->db->set($field,$value);}
		...
		$comments=$this->db->get_results('tbl_comments');
		...
		// Show all
		return array('view'=>'comments','errors'=>$errorHtml,'form'=>$formHtml,'items'=>$comments);
	}
	...
	function _check_if_double($data,$table) {
		...
		foreach ($data as $field => $value) $this->db->where($field,$value);
		$double=$this->db->get_row($table);
